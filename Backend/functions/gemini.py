...
def imageWithGemini(query:list,data:str,mime_type:str)->dict:
    ...
    try:
        ...
        model = genai.GenerativeModel('gemini-1.5-flash', system_instruction="Provide concise responses and limit them to 50 words.") # Initialize the GenerativeModel(gemini-1.5-flash model)
        safety_settings = {HarmCategory.HARM_CATEGORY_DANGEROUS_CONTENT: HarmBlockThreshold.BLOCK_ONLY_HIGH} # Set the safety settings to 'safe' for the image description
        ...
        if query:
            ...
            """
        # print(textPart)
        model_response = model.generate_content([textPart, filePart], generation_config=config, safety_settings=safety_settings) # Generate the image description
...
def videoWithGemini(query:list,data:str,mime_type:str)->dict:
    ...
    try:
        ...
        model = vgenai.GenerativeModel('gemini-1.5-flash', system_instruction="Provide concise responses and limit them to 50 words.") # Initialize the GenerativeModel(gemini-1.5-flash model)
